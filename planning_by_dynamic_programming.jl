...
# ╔═╡ af054d27-9b84-48af-8f2c-88ec691eb696
begin
	using PlutoUI
	using Plots
end
...
# ╔═╡ 905312cf-857b-4a5b-a2e3-f69ad4b9c0cb
md"""
Resources from [UCL Course on RL](https://www.davidsilver.uk/wp-content/uploads/2020/03/MDP.pdf)
"""

# ╔═╡ 1e3adfe6-edce-41d9-9b07-23095c840540
md"""
## Evaluating a Random Policy in the Small Gridworld
"""

# ╔═╡ bb60a94a-2065-48c8-9a35-7f241d4fca3b
PlutoUI.LocalResource("src/l3_1.png")
...
begin # Policy Evaluation
	...
	for k in 1:100 # Iterate for 100 times
...
function policy_improvement(ENV, v)
	GREEDY_POLICY = sort(Dict())
	for state in eachindex(v)
		opt_action = Vector{ACTIONS}()
		opt_action_val = -Inf
		for action in instances(ACTIONS)
...
begin # Policy Iteration
	...
	for iteration in 1:100
		...
		π = policy_improvement(ENV1, v)
		if (π == π_old) # Break if the policy is unchanged
			break
		end
	end
end
...
PlutoUI.LocalResource("src/l3_2.png")
...
# ╔═╡ e50a78d8-e1a7-409f-9e83-ce9e69174fc3
begin # Value Iteration
	v_env2 = zeros(4, 4)
	println("Initial value function :\n")
	show(stdout, "text/plain", v_env2)
	println()
	println()
	...
	for k in 1:100 # Iterate for 100 times
		...
		for state in eachindex(v_env2)
			state_action_val_max = -Inf
			for action in instances(ACTIONS)
				p, next_state, r, done = ENV2[state-1][action][1]
				state_action_val = (r + γ*p*v_env2[next_state+1])
				if state_action_val > state_action_val_max
					state_action_val_max = state_action_val
					v_copy[state] = state_action_val
				end
			end
		end
		
		if (abs.(v_env2 - v_copy) .< 0.01) == ones(size(v_env2)) # Break if the update is less than 0.01
			break
		end
		
		global v_env2 = v_copy
		println("Value function after iteration $(k) :\n")
		show(stdout, "text/plain", v_env2)
		println()
		println()
	...
	v_env2 = round.(v_env2, digits=4) # Round off value functions to 4 digits for comparison
end

# ╔═╡ 560289a8-c5b0-46c6-9c06-2d3717d88d20
md"""
#### Optimal Action Value Function
"""

# ╔═╡ 89353f6d-2cfd-4911-b90b-411cf7606513
heatmap(v_env2, color = :greys, yaxis = :flip, title="Optimal Action Value Function")
